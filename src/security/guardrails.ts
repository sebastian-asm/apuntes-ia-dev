...
const INJECTION_PATTERNS: Array<{ name: string; regex: RegExp }> = [
  ...
  },
  {
    ...
    regex: /forget\s+(everything|all|your\s+instructions?|what\s+you\s+were\s+told)/i
  },
  {
    name: 'you are now',
  ...
  // --- Español ---
  {
    ...
    regex: /ignora\s+(las\s+)?(instrucciones?\s+)?(anteriores?|previas?|todas?)/i
  },
  {
    name: 'olvida instrucciones (es)',
    regex: /olvida\s+(todo|las\s+instrucciones?|lo\s+que\s+te\s+(dijeron|indicaron))/i
  },
  {
    name: 'ahora eres (es)',
...
export function detectPromptInjection(input: string): { detected: boolean; pattern?: string } {
  for (const { name, regex } of INJECTION_PATTERNS) {
    if (regex.test(input)) return { detected: true, pattern: name }
  }
  return { detected: false }
}

export function checkGuardrails(input: string, rateLimiter: RateLimiter): GuardRailResult {
  ...
  if (!rateLimiter.check()) {
    ...
    return {
      safe: false,
      sanitized,
      reason: `Demasiadas solicitudes en poco tiempo\nEspera un momento antes de continuar (${rateLimiter.remaining})`
    }
  }
