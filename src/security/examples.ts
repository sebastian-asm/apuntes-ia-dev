import { sanitizeInput, detectPromptInjection, checkGuardrails, createRateLimiter } from './guardrails.js'
...
function printResult(
  ...
  const icono = result.safe === false || result.detected === true ? '⚠️  BLOQUEADO' : '✅ SEGURO'
  console.log(`\n--- ${title} ---`)
  ...
  if (result.sanitized && result.sanitized !== input) {
    console.log(`Sanitizado: "${result.sanitized.slice(0, 80)}${result.sanitized.length > 80 ? '...' : ''}"`)
  }
}
...
async function runExamples(): Promise<void> {
  ...
  printResult('Input truncado (9000 → 8000 chars)', `${'A'.repeat(50)}... (9000 chars)`, {
    safe: true,
  ...
  console.log('   En producción, checkGuardrails() se llama en cli.ts antes de agente.chat().\n')
}
